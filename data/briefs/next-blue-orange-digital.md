# Blue Orange Digital: Interview Notes and Prep

Onaolapo (Lapo) Odunjo. Role: Chief of Staff. Living notes doc, add to this after every conversation.

## Company and contacts, from your inbox

- Blue Orange Digital. blueorangedigital.com. No additional company description on file yet.
- Contact: Catarina Valente (recruiter, catarina.valente@blueorangedigital.na.teamtailor-mail.com)
- Next interview: Fri, Aug 7, 2026, 12:00 PM ET
- With: TBD (interviewer name not yet confirmed)

## What is known so far

- This is a Chief of Staff seat, which typically sits close to the founder or exec team
- Passed initial screening; now in final round
- No transcript or prep email available yet, so the brief is incomplete

## What is still unknown

- Company stage, funding, or core product focus
- Whether this Chief of Staff role leans operational, strategic, or both
- Who the interviewer is on Aug 7 and what they will be testing for
- Whether there are other final round participants or if this is a single conversation

## Next step prep (updated Aug 6, 2026): Final round, Fri Aug 7, 12:00 PM ET

### What this conversation is about

Chief of Staff final round. Interviewer name TBD but this is a final stage, which means they have already seen enough to move forward. Chief of Staff seats typically test for judgment under ambiguity, operational speed, and ability to work without a map.

This is thin on context, which is normal for a final that moved quickly. Use the first two minutes to calibrate what the interviewer cares about and what the role actually does day-to-day.

### What they are likely testing

- Judgment when structure is loose and stakes are real
- Speed and ownership in ambiguous environments
- Ability to build process while shipping, not instead of shipping
- Comfort operating close to the exec layer without formal authority
- Whether you can take incomplete signal, make a call, and keep things moving

Chief of Staff roles often fail when the person waits for permission or needs a clear map. They are looking for someone who can see what needs doing and do it.

### Lines and stories to land

Lead with the Kinage deployment story. It shows judgment, team leverage, and real outcomes in an ambiguous environment:

"I work best when the structure is loose and the stakes are real. At Kinage I built the deployment playbook from scratch while coaching engineers in the field. We went from zero to twelve institutions with cost-to-serve down about 62 percent. That was team leverage, not solo heroics."

If judgment under pressure comes up, use the TD pushback story. Calm alternative, protected delivery, spoke to risk and outcomes without escalating.

If they ask about regulated environments or production quality, talk about eval gates and the precision bar you held at the bank. Field failure modes into Product, not just into tickets.

If they ask about shipping with incomplete information, talk about cutting scope, shipping, measuring adoption, and iterating. Do not oversell this as a philosophy. It is just how you work when the spec is not done yet.

### Questions to ask

1. What does the Chief of Staff role own day-to-day, and where does it sit in the org?
2. What is the company working on right now, and what is the biggest operational or strategic gap?
3. What does success look like in the first 90 days?
4. What has made past Chiefs of Staff successful or unsuccessful here?

These are calibration questions, not interview questions. Ask them early so you can adjust your answers to what they actually care about.

### Action items before the call

1. Confirm the interviewer's name and role in the first minute
2. Have the Kinage deployment story and the TD pushback story ready as your two strongest proof points
3. Keep answers short and grounded in real outcomes, not hypotheticals
4. After the call, add notes here immediately so the next conversation or decision has full context

## Interview line to reuse, in their own language

"I work best when the structure is loose and the stakes are real. I take what signal exists, make a call, and keep things moving. At Kinage I built the deployment playbook from scratch while coaching engineers in the field. We went from zero to twelve institutions with cost-to-serve down about 62 percent. That was team leverage, not solo heroics."

## Proof points to have ready

- Multi-account AI deployment at Kinage: 0 to 12 institutions, playbooks written live, coached deployed engineers, cost-to-serve dropped roughly 62 percent
- Regulated environment work: eval gates, precision bar, production quality in banking context
- Judgment under pressure: TD-style pushback story, calm alternative proposed, protected delivery while speaking to risk
- Shipping with incomplete information: cut scope, ship, measure adoption, iterate

## How to read this and what to do with it

This is thin on context, which is normal for a final round that moved quickly. A few things worth noticing:

- Chief of Staff roles test for judgment, speed, and ability to operate without a map
- Final round suggests you are close to a decision
- No prior notes means you should ask clarifying questions early in the conversation to calibrate

After the call, come back here and add what you learned about the company, what the interviewer cared about, and what the role actually does. That way if there is a follow-up conversation or a decision call, you have the full picture in one place.

This is a living document. Add new notes here after every Blue Orange Digital conversation so the next round always has the full picture in one place.
